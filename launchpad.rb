...
class Launchpad
  ...
  @servers = {
    "uat" =>{
      ...
      "parent_places" => {
        "api/" => "https://developer-uat.motorolasolutions.com/api/core/v3/places/24017",
        "guide/" => "https://developer-uat.motorolasolutions.com/api/core/v3/places/24013",
        "guide/profiles/" => "https://developer-uat.motorolasolutions.com/api/core/v3/places/24018",
        "guide/reference/" => "https://developer-uat.motorolasolutions.com/api/core/v3/places/24017",
        "guide/sample/" => "https://developer-uat.motorolasolutions.com/api/core/v3/places/24016",
        "guide/tutorial/" => "https://developer-uat.motorolasolutions.com/api/core/v3/places/24015",
      }
    },
    "prod" =>{
      "server" => "developer.motorolasolutions.com/api/core/v3",
      "parent_places" => {
        "api/" => "https://developer.motorolasolutions.com/api/core/v3/places/24017",
        "guide/" => "https://developer.motorolasolutions.com/api/core/v3/places/24013",
        "guide/profiles/" => "https://developer.motorolasolutions.com/api/core/v3/places/24018",
        "guide/reference/" => "https://developer.motorolasolutions.com/api/core/v3/places/24017",
        "guide/sample/" => "https://developer.motorolasolutions.com/api/core/v3/places/24016",
        "guide/tutorial/" => "https://developer.motorolasolutions.com/api/core/v3/places/24015",
      }
    }
  ...
  def self.generate_html(topic,parent_source)
    ...
    md = replace_featurecompatibility(md)
    ...
    if topic.start_with?('docs/api')
      basename = topic.gsub(AppConfig['api_md'],'')
      ...
      parent_output = Pathname(parent_source).each_filename.to_a.last
    	outputfile = "#{AppConfig['launchpad_html']}#{parent_output}/#{basename.gsub!('.md','.html')}"
  ...
  def self.generate_blog_html(topic,parent_source)
    #open Markdown content
    md = File.read(topic)
    ...
    # Add Markdown converted to HTML
      ...
      # Need to add to Launchpad theme on doc load:
      # dp.SyntaxHighlighter.HighlightAll()
      html.gsub!(/<pre><code> :::(\w+)\s*(\n|&#x000A;)/, '<pre name="code" class="\1" style="background-color:white">')
      html.gsub!('</code></pre>','</pre>')
      ...
      basename = topic.gsub(parent_source,'')
      parent_output = Pathname(parent_source).each_filename.to_a.last
      outputfile = "#{parent_output}/#{basename.gsub!('.md','.html')}"
      File.open("#{outputfile}", 'w') {|f| 
        f.write(html) 
        # puts outputfile
      }
 ...
 def self.replace_url_md (md)
    ...
    md_mod = md.gsub(/\[.*?\]\((.*?)\)/) do |m|
      ...
      if index_key.include?('?')
        ...
        index_key = index_key.split("?").first
        
        link_jump = "#{match.split("#").last}"
      end      
      # otherwise use the match for the lookup
      if url_map[index_key].nil? 
        if !index_key.start_with?('http')
          puts "\nUnrecognized URL #{index_key}"
        end
        # do nothing, myable external url or one we do not know
        m
      else
  ...
  def self.replace_featurecompatibility(md)
    md_mod = md
    md_mod = md.gsub('## Feature Compatibility','')
    return md_mod
  end
    ...
    md_mod = md.gsub(/\[.*?\]\((.*?)\)/) do |m|
       ...
       if !match.start_with?('http')   
        if match.start_with?('#')
          ...
          if url_map[index_key].nil? || url_map[index_key]["url"][env] == ""
            puts "\nERROR: #{index_key} missing url"
            m
          else
            # get the lookup for the real LP url
  ...
  def self.publish_html(topic,html,url_map,env)
    ...
    basename = topic.gsub(AppConfig['launchpad_html'],'')
  ...
  def self.generate_mapping_index
    ...
    puts "Generating JSON Index: #{AppConfig['launchpad_html']}#{AppConfig['launchpad_mapping']}"
    ...
    apiFiles.each do |fileName|
      ...
      puts "Parent: #{parent}"
      index_key = parent + "/" + fileName.gsub(AppConfig['api_md'],'').gsub('.md','')
    ...
    guidesFiles.each do |fileName|
      ...
      parent = Pathname(fileName.gsub(basename,'')).each_filename.to_a.last
      index_key = parent + "/" + fileName.gsub(AppConfig['guides_md'],'').gsub('.md','')
    ...
    if File.file?(outputfile)
      puts "**** WARNING: previous mapping existed - last one saved in #{AppConfig['launchpad_mapping']}.backup"
      #save copy
