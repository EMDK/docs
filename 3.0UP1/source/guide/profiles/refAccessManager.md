...
> WARNING: Be sure to always include the EMDK for Android service package name `com.symbol.emdkservice` when this feature is enabled. Otherwise none of the EMDK APIs will work. Specifying the com.symbol.emdk.emdkservice in `packages allowed to submit XML` allows all the applications to submit XML - this behavior will be changed in a future release. 
